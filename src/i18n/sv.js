export default { 
 error: { 
   forgotPassword: { 
     "too_many_requests": "Du har nått gränsen för maximalt antal försök att ändra ditt lösenord. Var vänlig vänta innan du försöker igen.", 
     "lock.fallback": "Något gick fel när vi försökte ändra ditt lösenord." 
   }, 
   login: { 
     "blocked_user": "Användaren är spärrad.", 
     "invalid_user_password": "Fel på inloggningsuppgifter.", 
      "lock.fallback": "Någit gick fel vid inloggningsförsöket.", 
      "lock.invalid_code": "Fel kod.", 
      "lock.invalid_email_password": "Fel email eller password.", 
      "lock.invalid_username_password": "Fel användarnamn eller lösenord.", 
      "lock.network": "Kunde inte kontakta servern. Var vänlig kontrollera din uppkoppling och försök igen.", 
      "lock.popup_closed": "Popup-fönster stängt. Försök igen.", 
      "lock.unauthorized": "Åtkomst nekad. Försök igen.", 
      "password_change_required": "Du behöver ändra ditt lösenord då detta är första gången du loggar in eller för att ditt lösenord har gått ut.", // TODO: verify error code 
      "password_leaked": "Din konto har blivit låst då ditt lösenord har lagts upp på en annan webbsite. Du har fått ett email med instruktioner om hur du låser upp ditt konto.", 
      "too_many_attempts": "Ditt konto har blivit låst efter att flera felaktiga inloggningsförsök har gjorts." 
    }, 
    passwordless: { 
      "bad.email": "Din email-adress är ogiltigt.", 
      "bad.phone_number": "Ditt telefonnummer är ogiltigt.", 
      "lock.fallback": "Något gick fel." 
    }, 
    signUp: { 
      "invalid_password": "Lösenordet är ogiltigt.", 
      "lock.fallback": "Något gick fel vid registreringen.", 
      "password_dictionary_error": "Lösenordet är för vanligt.", 
      "password_no_user_info_error": "Lösenordet baseras på personuppgifter.", 
      "password_strength_error": "Lösenordet är för svagt.", 
      "user_exists": "Användaren finns redan.", 
      "username_exists": "Användarnamnet finns redan." 
    } 
  }, 
  success: { // success messages show above the form or in a confirmation pane 
    logIn: "Tack för att du loggar in.", 
    forgotPassword: "Vi har skickat dig ett email där du kan återställa ditt lösenord.", 
    magicLink: "Vi har skickat en länk för att logga in<br />till %s.", 
    signUp: "Tack för att du registrerade dig." 
  }, 
  blankErrorHint: "Får inte lämnas tom", 
  codeInputPlaceholder: "Din kod", 
  databaseEnterpriseLoginInstructions: "", 
  databaseEnterpriseAlternativeLoginInstructions: "eller", 
  databaseSignUpInstructions: "", 
  databaseAlternativeSignUpInstructions: "eller", 
  emailInputPlaceholder: "dinmailadress@exempel.se", 
  enterpriseLoginIntructions: "Logga in med dina företagsuppgifter.", 
  enterpriseActiveLoginInstructions: "Ange dina företagsuppgifter för %s.", 
  failedLabel: "Misslyckades!", 
  forgotPasswordAction: "Har du glömt ditt lösenord?", 
  forgotPasswordInstructions: "Ange din mailadress. Vi skickar ett mail med instruktioner för att återställa ditt lösenordet.", 
  forgotPasswordSubmitLabel: "Skicka email", 
  invalidErrorHint: "Ogiltig", 
  lastLoginInstructions: "Senast du loggade in med", 
  loginAtLabel: "Logga in på %s", 
  loginLabel: "Logga in", 
  loginSubmitLabel: "Logga in", 
  loginWithLabel: "Logga in med %s", 
  notYourAccountAction: "Inte ditt konto?", 
  passwordInputPlaceholder: "ditt lösenord", 
  passwordStrength: { 
    containsAtLeast: "Måste innehålla minst %d av följande %d typer av tecken:", 
    identicalChars: "Inte fler än %d identiska tecken i rad (t.ex., \"%s\" är inte tillåtet)", 
    nonEmpty: "Lösenord får inte vara tomma", 
    numbers: "Nummer (t.ex. 0-9)", 
    lengthAtLeast: "Minst %d tecken långt", 
    lowerCase: "Gemener (a-z)", 
    shouldContain: "Måste innehålla:", 
    specialCharacters: "Specialtecken (t.ex. !@#$%^&*)", 
    upperCase: "Versaler (A-Z)" 
  }, 
  passwordlessEmailAlternativeInstructions: "Annars kan du ange din emailadress för att logga in<br>eller skapa ett konto", 
  passwordlessEmailCodeInstructions: "Ett email med din kod har skickats till %s.", 
  passwordlessEmailInstructions: "Ange din emailadress för att logga in<br>eller skapa ett konto", 
  passwordlessSMSAlternativeInstructions: "Annars kan du ange ditt telefonnummer för att logga in<br>eller skapa ett konto", 
  passwordlessSMSCodeInstructions: "Ett SMS med din kod har skickats<br>till %s.", 
  passwordlessSMSInstructions: "Ange ditt telefonnummer för att logga in<br>eller skapa ett konto", 
  phoneNumberInputPlaceholder: "ditt telefonnummer", 
  resendCodeAction: "Fick du ingen kod?", 
  resendLabel: "Skicka om", 
  resendingLabel: "Skickar om...", 
  retryLabel: "Försök igen", 
  sentLabel: "skickats!", 
  signUpLabel: "Registrera dig", 
  signUpSubmitLabel: "Registrera dig", 
  signUpTerms: "", 
  signUpWithLabel: "Registrera dig med %s", 
  socialLoginInstructions: "", 
  socialSignUpInstructions: "", 
  ssoEnabled: "Single Sign-On aktiverad", 
  submitLabel: "Skicka", 
  unrecoverableError: "Något gick fel.<br />Var vänlig kontakta teknisk support.", 
  usernameFormatErrorHint: "Använd 1-15 tecken, siffror och \"_\"", 
  usernameInputPlaceholder: "ditt användanamn", 
  usernameOrEmailInputPlaceholder: "användarnamn/email", 
  title: "Auth0", 
  welcome: "Välkommen %s!", 
   windowsAuthInstructions: "Du är uppkopplad från ditt företagsnätverk&hellip;", 
   windowsAuthLabel: "Windows Authentication" 
 }; 
