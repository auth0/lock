...
- **socialButtonStyle {String}**: Determines the size of the buttons for the social providers. Possible values are `"big"` and `"small"`. The default style depends on the connections that are available:
  ...
  - Keep in mind that some branding guidelines don't allow for icons without text. If you want to be compliant with those guidelines for any reason, make sure you pick the right style for your application.
